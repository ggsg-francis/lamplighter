...
namespace graphics
{
	...
	Mesh ProcessMesh(aiMesh *mesh, const aiScene *scene)
	{
		...
		for (unsigned int i = 0; i < mesh->mNumVertices; i++)
		{
			...
			// set matrix values to their default
			vertex.matr[0u] = 0.f;
			vertex.matr[1u] = 0.f;
			vertex.matr[2u] = 0.f;
			vertex.matr[3u] = 0.f;
	...
	void ConvertMD(char* sfn, void* OUT, MDHandleType type)
	{
		...
		for (btui32 iVert = 0; iVert < a.vertices.size(); ++iVert)
		{
			...
			case graphics::eDEFAULT:
				break;
			case graphics::eBODY:
				//vces[iVert].pos.y -= (bodlen * 0.5f) * vces[iVert].mat[0];
				break;
			case graphics::eARM:
				break;
			case graphics::eHEAD:
				break;
			case graphics::eLEG:
				//vces[iVert].pos.x -= hipw * (1.f - vces[iVert].mat[0]);
				//vces[iVert].pos.y += (leglen * 0.5f) * (vces[iVert].mat[1] - vces[iVert].mat[2]);
				//vces[iVert].pos.y += leglen * vces[iVert].mat[2];
				break;
			}
		}
		// Copy indices straight from mesh A, as they should be identical in each mesh
