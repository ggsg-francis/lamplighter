...
namespace network
{
	...
	void Recv2(int type)
	{
		...
		if (recvd > 0)
		{
			...
			{
			case SET_UNIT_POSE:
				msg::set_unit_pose msup;
				memcpy(&msup, pw.msg, sizeof(msg::set_unit_pose));
				//print the message
				std::cout << "TYPE: " << (int)pw.type << " INPUT: " << msup.px << ", " << msup.py << ", " << msup.yaw << std::endl;
				//index::players[1]->SetPos(fw::Vector3(msup.px, msup.py, msup.pz));
				//index::player2->SetView(msup.vh, msup.vv);
				index::SetPose(&msup);
				break;
			case SPAWN_PROJECTILE:
				...
				break;
			case SERVER_CONNECT_CLIENT:
				msg::server_connect_client mscc;
				memcpy(&mscc, pw.msg, sizeof(msg::server_connect_client));
				nid = mscc.id;
				SetTitle(mscc.id);
				std::cout << "Server set our ID to " << mscc.id << std::endl;
				break;
	...
	void Init()
	{
		...
		*/
		...
		//SOCKET connHandle = socket(AF_INET, SOCK_STREAM, NULL); //set connection socket
