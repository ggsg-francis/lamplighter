...
{    
	...
	vec4 color = texture(texture_diffuse1, vec2(TexCoords.x, TexCoords.y + (ft * 2.f)));
	if (color.a - (1 - Col.r) < 0.5)
	{
		discard;
	};
    FragColor = texture(texture_diffuse1, vec2(TexCoords.x, TexCoords.y + (ft * 2.f)));
	...
	// Add fog
	FragColor.rgb = mix(FragColor.rgb, cfog, clamp(length((Pos - pcam) * 0.1f), 0.f, 1.f));
