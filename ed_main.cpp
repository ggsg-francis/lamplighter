...
Texture rendertexture_1;
Texture depthbuffer_1;
...
int main(int argc, char * argv[])
{
	...
	* You may need to change this to 16 or 32 for your system */
	...
	//Shader shad_gui("shaders/gui.vs", "shaders/gui.fs");
	...
	#endif // !DEF_OLDSKOOL
	#ifndef DEF_OLDSKOOL
	//texture_test.id = TxLoadTGA("crosshairs_hifi.tga", "resources");
	texture_test.id = TxLoadTGA("crosshairs.tga", "resources");
	#endif // !DEF_OLDSKOOL
	...
	while (!quit)
	{
		...
		//draw ui
		pRes->shader_gui.Use();
		guibmp.SetTexture(res::GetBTT(res::t_equip_atlas));
		//guibmp.SetTexture(pRes->tex_test);
		//guibmp.Draw(p1_x_start + 32, p1_y_start + 16, index::players[0]->hp * 64, 32);
		//guibmp.Draw(p2_x_start + 32, p2_y_start + 16, index::player2->hp * 64, 32);
